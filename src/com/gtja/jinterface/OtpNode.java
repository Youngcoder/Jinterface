...
import com.gtja.jinterface.copy.OtpNode.Mailboxes;
...
public class OtpNode extends OtpLocalNode {
     ...
     * 
     */
    public OtpNode(final String node) throws IOException, OtpErlangException {
     ...
     * 
     */
    public OtpNode(final String node, final String cookie) throws IOException, OtpErlangException {
	this(node, cookie, 0);
    }
	...
	if (conn != null && conn.name != null) {
	    ...
	    GlobalNodesView.addNode(conn.peer);
	...
	if (conn != null && conn.name != null) {
	    ...
	    GlobalNodesView.removeNode(conn.name);
	}
    }
